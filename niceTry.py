...
def isCircle(tempPoints):
    if len(tempPoints) <= 16:  # drawing circles make a lot more points
        return [False]
    count = 0
    for i in range(len(tempPoints)):
        for j in range (len(tempPoints)):
            if abs(i - j) <= 10:  # each point compared must be at least 11 points apart
                continue
            elif abs(tempPoints[i][0] - tempPoints[j][0]) <= 18 and abs(tempPoints[i][1] - tempPoints[j][1]) <= 18:
                count += 1
            if count >= 10:  # at least 10 such points must be close to one another
